...
class Parser:
    ...
    def yadi_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'YADI'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'YADI'"
            ))
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'TADA'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'TADA'"
            ))
        ...
        while self.current_tok.matches(TT_KEYWORD, 'ANYE'):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            yadi_expr = res.register(self.yadi_expr())
            if res.error: return res
            return res.success(yadi_expr)
    ...
    def bin_op(self, func_a, ops, func_b=None):
        ...
        while self.current_tok and self.current_tok.type in ops or (self.current_tok.type, self.current_tok.value) in ops:
            op_tok = self.current_tok
            res.register_advancement()
            self.advance()
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.substracted_by(right)
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_MINUS:
            ...
            number, error = number.anded_by(Number(0))
